...
const Transformer = {
  ...
  getModelFromJsDoc (sails, doc, isRequest) {
    ...
    if (!modelTag) return;
    let [ $, modelNameToFind ] = /^\[?(.+?)]?$/.exec(modelTag.value.toLowerCase());
    return _.find(sails.models, function (model, modelName) {
      return modelName.toLowerCase() === modelNameToFind;
    });
  },

  isModelArray (sails, methodGroup, jsDoc, isRequest) {
  ...
  getModelIdentityFromRoute (sails, methodGroup, jsDoc, isRequest) {
    var doc = Transformer.getJsDocFromRoute(sails, methodGroup, jsDoc);
    let model = null;
    if (doc) {
  ...
  getDefinitionSchemaFromRoute (sails, methodGroup, jsDoc, isRequest) {
    let reference = Transformer.getDefinitionReferenceFromRoute(sails, methodGroup, jsDoc, isRequest);
    if (reference) {
      let schema = {'$ref': reference};
      let isArray = Transformer.isModelArray(sails, methodGroup, jsDoc, isRequest);
      if (isArray) {
        schema = Spec.wrapInArray(schema);
      }
      return schema;
    }
  },
  ...
  getResponses (sails, methodGroup, jsDoc) {
    let schema = Transformer.getDefinitionSchemaFromRoute(sails, methodGroup, jsDoc, false);
    let ok = {
